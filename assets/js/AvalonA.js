...
let _TweenLite = undefined;

let noeffect = rotation=> rotation;
let transformStyleIsSupported = undefined;
...
let defineAvalonA = function() {
	class Frame3d {
		...
		onrotation() {
			...
			if (this.idleTimeout > 0) { return this.rotationTimeoutId = setTimeout(
				this.stopRotation,
				this.idleTimeout
			); }
		}
		...
		init(options){
			...
			return Object.defineProperties(
				this,{
				onstart: {
					get() { return this.onenable; },
					set(onstart){ return this.onenable = onstart; }
				}
			}
			);
		}
